...
class AppSettings:
    ...
    #### make dfs
    def make_scheduler_selections_df(self):
        df = pd.DataFrame(self.dask_scheduler_options_selected)[['arg', 'default']]
        convert_to_str = lambda x: '/'.join(x) if type(x) == list else x
        df['arg'] = df['arg'].apply(convert_to_str)
        df = df.set_index('arg')
        self.scheduler_df = df

    def make_worker_selections_df(self):
        df = pd.DataFrame(self.dask_worker_options_selected)[['arg', 'default']]
        convert_to_str = lambda x: '/'.join(x) if type(x) == list else x
        df['arg'] = df['arg'].apply(convert_to_str)
        df = df.set_index('arg')
        self.worker_df = df

    def make_cuda_worker_selections_df(self):
        df = pd.DataFrame(self.dask_cuda_worker_options_selected)[['arg', 'default']]
        convert_to_str = lambda x: '/'.join(x) if type(x) == list else x
        df['arg'] = df['arg'].apply(convert_to_str)
        df = df.set_index('arg')
        self.cuda_worker_df = df
